...
public interface CarsDtModelRepository extends JpaRepository<CarsDtModel,String>{
	...
	@Query(value=" select DT_ID from CARS_DT_MODEL where INS_ID  =  ?1  AND  upper(TRIM(MODEL_NAME)) = upper(TRIM(?2)) AND upper(TRIM(INS_MAKE_CODE)) = upper(TRIM(?3)) ",nativeQuery = true)
